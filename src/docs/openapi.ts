...
const localServer = { url: 'http://localhost:4000', description: 'Local development' };
const productionServer = deployedBaseUrl
  ? { url: deployedBaseUrl, description: 'Production (Render)' }
  : undefined;

const openApiServers =
  process.env.NODE_ENV === 'production' && productionServer
    ? [productionServer, localServer]
    : [localServer, ...(productionServer ? [productionServer] : [])];
...
export const openApiSpec = {
  ...
  info: {
    ...
    version: '1.0.0',
    description:
      'Finance data processing and role-based access control API for dashboard, users, and records.',
    contact: {
      name: 'Biplab Mal',
